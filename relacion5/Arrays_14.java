...
public class Arrays_14 {
	...
	static int[] bolasNuevas(int[] v, int random) { // introduce un nuevo valor al array original
		
		int totalV2 = v.length + 1; // creo un array nuevo con el tamaño del array original + 1 del nuevo numero a introducir
		int[] v2 = new int[totalV2];
		
		for (int i = 0; i < v.length; i++) { // copio el contenido del array original en el array nuevo
			v2[i] = v[i];
		}
		
		v2[v2.length - 1] = random; // añado el numero nuevo a la última posición del array
		
		return v2; // devuelvo el array al main
	}
	...
	static int[] noRepetidos(int[] v) { // crea un array de Bingo con nºs aleatorios no repetidos
		Random random = new Random();
		
		boolean encontrado = false;
		int r, indice = 0, j = 0;
		
		while (indice < v.length) { // recorro el array del Bingo para buscar nºs repetidos
									// compruebo posición por posición
			encontrado = false;
			r = random.nextInt(1, 91); // genero un nº aleatorio
			j = 0;
			
			while (!encontrado && j < indice) { // recorro el array desde 0 hasta la posición actual
				
				if (v[j] == r)	// si el nº actual es igual al generado arriba, paro el bucle interno porque hay una coincidencia
					encontrado = true;
		...
		return v; // devuelvo un array aleatorio con nºs únicos al main
	}
	...
	public static void main(String[] args) {
		...
		int aleatorio = 0, numeroUsuario;
		boolean detener = false, comprobar, aleatorioYaExiste, generarRandom = true;
		String respuesta;
		
		numBingo = noRepetidos(numBingo); // llamo al método para crear un bingo con valores no repetidos
		...
		while (!detener) {
			System.out.println("¿(S)iguiente bola, (C)omprobar bingo o (T)erminar?"); // entramos al bucle para generar más nºs, comprobar bingo o terminar programa
			respuesta = sc.next();													  // recojo "S", "C", o "T"
			
			if (respuesta.compareTo("S") == 0){
				generarRandom = true;
				
				while (generarRandom) { // genero un nº random y lo envío al método para comprobar si es nº existe o no en el bingo
				...
				numNuevos = bolasNuevas(numBingo, aleatorio); // cuando el nº generado no esté repetido, llamo al método para insertar el nuevo valor en el array
				...
				if (comprobar) {
					System.out.println("¡¡BINGO!! Enhorabuena, has ganado un premio."); // según la respuesta del método, paro el programa o continúo al principio del bucle
					detener = true;
				} else {
					System.out.println("No tienes bingo, lo siento. Sigue jugando.");
				}
				
			} else if (respuesta.compareTo("T") == 0) { // paro el programa
