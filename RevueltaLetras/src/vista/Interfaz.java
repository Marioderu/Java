...
public class Interfaz {
	...
	public boolean preguntarPista(int pistasRestantes) {
		...
		System.out.println("Tienes " + pistasRestantes + " pistas restantes por usar. ¿Te gustaría usar una pista? Escribe S / N:");
		
		while (!detener && !quierePista) {
			
			pista = sc.nextLine();
			
			if (pista.length() == 1 && (pista.charAt(0) == 'S' || pista.charAt(0) == 's' || pista.charAt(0) == 'N' || pista.charAt(0) == 'n')) {
				
				if (pista.charAt(0) == 'S' || pista.charAt(0) == 's') {
				...
				System.out.println("Has introducido un valor no admitido. Debes introducir 'S' (quieres una pista) o 'N' (no quieres pista)");
			}
		}
	...
	public int preguntarRanking(int num) {
		...
		while (!detener) {
			...
			if (n.length() == 1 && (n.charAt(0) >= 48 && n.charAt(0) <= 51)) {
