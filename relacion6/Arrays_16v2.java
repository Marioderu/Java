...
public class Arrays_16v2 {
	
	static int[] partesIndividuales() { // creo un array con 3 nºs aleatorios que servirán como posición para cada uno de los 3 array principales
		Random r = new Random();
		
		int aleatorio;
		int[] posiciones = new int[3];
		
		for (int i = 0; i < 3; i++) { // genero 3 nºs y los inserto en un array
	...
	public static void main(String[] args) {
		...
		int longitud = 10, fabricado, combinacion; // inicio variables y arrays
		int[] partesInd;
		...
		for (int i = 0; i < cabezas.length; i++) {
			partesInd = partesIndividuales();									// llamo a los métodos para generar 3 nº aleatorios
			combinacion = cabezaCombinada(partesInd, ojos, narices, sonrisas);  // asignarlos a un valor contenido en cada uno de los 3 arrays anteriores y formar el nº de 3 dígitos
